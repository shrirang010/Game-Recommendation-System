...
with open('metrics3.csv', 'r') as file:
    ...
    for document in reader:
        ...
        if (float(document[1]) >= 0 and float(document[1]) < 0.1):  # very negative
            text = "Worst"
            count1 += 1
        elif (float(document[1]) >= 0.1 and float(document[1]) < 0.15):  # negative
            text = "Very Poor"
            count2 += 1
        elif (float(document[1]) >= 0.15 and float(document[1]) < 0.20):  # partially negative
            text = "Poor"
            count3 += 1
        elif (float(document[1]) >= 0.20 and float(document[1]) < 0.25):  # mixed
            text = "Average"
            count7 += 1
        elif (float(document[1]) >= 0.25 and float(document[1]) < 0.4):  # partially positive
            text = "Good"
            count4 += 1
...
with open('metrics.csv', 'w') as file:
    ...
    writer.writerow(["id","metric","review"])
